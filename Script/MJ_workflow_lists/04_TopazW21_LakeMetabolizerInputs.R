...
year <- 2021 #desired open water period
...
load("04_Topaz_2021_winterdata_corr_forMJ.rdata")
...
sensor_data <- sensor_data[with(sensor_data, datetime >= "2021-06-01 14:00:00" & datetime <= "2021-08-21 17:00:00"),]
...
cond_depth <- strsplit(names(sensor_data)[grep('cond',names(sensor_data))],"_")
cond_depth <- as.numeric(cond_depth[[1]][2])
cond_depth
#remove temperature reading from conductivity sensor
if(!is.na(cond_depth)){
  sensor_data <-  sensor_data[,-grep(paste('temp',cond_depth,sep="_"),names(sensor_data))]
}
...
sensor_data$shading = ifelse(sensor_data$light_2.08 > 10000, 1, 0)
...
wd <- "C:/Users/maryj/Documents/R/SierraPonds/PondMetabolism/Data"
setwd(wd)
year <- 'W2021' #desired open water period
