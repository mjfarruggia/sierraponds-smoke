...
) #REMOVED PM2.5 AND SMOKE MODELS FOR 2022
...
C.list <- list(C_6.1, C_6.2_sh, C_6.2_sep) #ADJUSTED THIS SO IT DOESN'T INCLUDE PM2.5 or smoke
C.list[[3]]
...
combos[,7] <- c(rep(1,3),rep(2,6)) #ADJUSTED SO IT DOESN'T INCLUDE PM2.5 ROWS
#C
#combos[,8] <- c(rep(1,3), rep(2,3), rep(3,3),rep(4,3), rep(5,3),rep(6,3),rep(7,3))
combos[,8] <- c(rep(1,3), rep(2,3), rep(3,3)) #ADJUSTED SO IT DOESN'T INCLUDE PM2.5 ROWS
...
save(mod.output, file="Tokopah_2022_MARSS_output_03262024.Rdata")
